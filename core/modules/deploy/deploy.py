...
class Deployer(Workers):
    ...
    def compileTemplate(self):
        script_name = "syz-compile.sh"
        if self.arch == "arm64":
            script_name = "syz-compile-arm64.sh"
        target = os.path.join(self.package_path, "scripts/{}".format(script_name))
        chmodX(target)
        self.logger.info("run: scripts/{}".format(script_name))
    ...
    def __run_delopy_script(self, hash_val, case, kasan_patch=0):
        ...
        with p.stdout:
            self.__log_subprocess_output(p.stdout, logging.INFO)
        exitcode = p.wait()
        self.logger.info("script/{} is done with exitcode {}".format(script_name, exitcode))
        return exitcode
