...
class CrashChecker:
    def __init__(self, project_path, case_path, ssh_port, logger, debug, offset, qemu_num, arch="amd64", store_read=True, compiler="gcc-7", max_compiling_kernel=1):
        ...
        self.package_path = os.path.join(project_path, "core")
        self.case_path = case_path
        self.basic_info_folder = os.path.join(self.case_path,'basic_info')
    ...
    def run(self, syz_repro, syz_commit, log=None, linux_commit=None, config=None, c_repro=None, i386=None):
        ...
        self.case_logger.info("[Crash] Found {} existed crashes".format(len(crashes_path)))
        for path in crashes_path:
            self.case_logger.info("[Crash] Inspect crash: {}".format(path))
    ...
    def repro_on_fixed_kernel(self, syz_commit, linux_commit=None, config=None, c_repro=None, i386=None, patch_commit=None, crashes_path=None, limitedMutation=False):
        if crashes_path == None:
            ...
            if len(crashes_path) == 0:
                return []
        self.case_logger.info("to repro case:")
        for path in crashes_path:
            self.case_logger.info("{}".format(path))
        ...
        for path in crashes_path:
            ...
            if ori_crash_report != []:
                if trigger:
                    ...
                    self.case_logger.info("[Crash] Crash {} in vulner_version reproduce [CONFIRM]".format(key))
                else:

                    reproduceable[key] = SUSPICIOUS
                    self.case_logger.info("[Crash] Crash {} in vulner_version reproduce [SUSPICIOUS]".format(key))
            else:

                reproduceable[key] = NONCRASH
                self.case_logger.info("[Crash] Crash {} in vulner_version reproduce [NONCRASH]".format(key))

        exitcode = self.deploy_linux(patch_commit, config, 1)
        if exitcode == 1:
            self.logger.info("Error occur at deploy_linux.sh when apply the patch")
            self.case_logger.info("[Crash] Error occur at deploy_linux.sh when apply the patch")
            return res
        self.case_logger.info("[Crash] Success apply the patch")
        for path in crashes_path:
            key = os.path.basename(path)
            path_repro = os.path.join(path, "repro.prog")
            ...
            if ori_crash_report != []:
                if trigger:
                    ...
                    self.case_logger.info("[Crash] Reproduceable: {} in patch_version".format(key))
                else:

                    if reproduceable[key] == CONFIRM:

                        self.logger.info("Slightly Fixed: {}".format(key))
                        self.case_logger.info("[Crash] Slightly Fixed: {}".format(key))
                        res.append(path)
                ...
                if reproduceable[key] == SUSPICIOUS:
                    ...
                    self.case_logger.info("[Crash] Suspicious crash: {} triggered a crash but doesn't belong to OOB/UAF write".format(key))

        exitcode = self.deploy_linux(linux_commit, config, 0)
        self.case_logger.info("find crash num:{}\tcan repro num:{}\treal check num:{}".format(
            crash_find_num,
            len(crashes_path),
            len(res)))
        return res
        ...
        if os.path.isdir(crash_path):
            for i in range(0,99):
                ...
                if os.path.isdir(crash_path_tmp):
                    ...
                    break
        self.case_logger.info("use latest crashes path:{}".format(crash_path))
        res = []
        crash_find_num = 0
        if os.path.isdir(crash_path):
    ...
    def read_from_log(self, log):
        ...
        record_flag = 0
        kasan_flag = 0
        with open(os.path.join(self.basic_info_folder,'log'), 'r') as f:
            log_save = f.read()
        text = log_save.split('\n')
        for line in text:
    ...
    def deploy_linux(self, commit, config, fixed):
        ...
        if self.arch == "arm64":
            script_name = "deploy_linux_arm64.sh"
        target = os.path.join(self.package_path, "scripts/{}".format(script_name))
        utilities.chmodX(target)
        p = None
                ...
                if extract_report:
                    ...
                    for line in qemu.output[out_begin:]:
                        if utilities.regx_match(call_trace_regx, line) or utilities.regx_match(message_drop_regx, line):
                            record_flag = 1
                        if utilities.regx_match(boundary_regx, line) or utilities.regx_match(panic_regx, line):
                            if record_flag == 1:
                                res.append(crash)
                                crash = []
                                if kasan_flag and (write_flag or read_flag or double_free_flag):
                                    ...
                                    if write_flag:
                                        ...
                                        self.case_logger.info("QEMU threaded {}: OOB/UAF write triggered".format(th_index))
                                        qemu.kill_qemu = True
                                    if double_free_flag:
                                        self.logger.debug("QEMU threaded {}: Double free triggered".format(th_index))
                                        self.case_logger.info("QEMU threaded {}: Double free triggered".format(th_index))
                                    if read_flag:
                                        self.logger.debug("QEMU threaded {}: OOB/UAF read triggered".format(th_index))
                                        self.case_logger.info("QEMU threaded {}: OOB/UAF read triggered".format(th_index))
                                    break
                            record_flag = 1
                            continue
    ...
    def upload_exp(self, syz_repro, port, syz_commit, repro_type, c_repro, i386, fixed, logger):
        script_name = "upload-exp.sh"
        if self.arch == "arm64":
            script_name = "upload-exp-arm64.sh"
        target = os.path.join(self.package_path, "scripts/{}".format(script_name))
        utilities.chmodX(target)
    ...
    def make_commands(self, text, support_enable_features, i386):
        ...
        normal_pm = {"arch":self.arch, "threaded":"false", "collide":"false", "sandbox":"none"}
        for line in text:
            ...
            if line.find('{') != -1 and line.find('}') != -1:
                ...
                try:
                    pm_start = line.find('{')
                    pm = json.loads(line[pm_start:])

                except json.JSONDecodeError:
                    pm = utilities.syzrepro_convert_format(line[pm_start:])
    ...
    def monitor_execution(self, p):
        ...
        while (count <60):
            if self.kill_qemu:
                self.case_logger.info('[Crash] Signal kill qemu received.')
                p.kill()
                return
            count += 1
            time.sleep(10)
    ...
    def __match_allocated_section(self, report1 ,report2):
        ...
        self.case_logger.info("[Crash] diff ratio: {}".format(ratio))
        if ratio > 0.3:
            return [False, ratio]
        return [True, ratio]
    ...
    def __log_subprocess_output(self, pipe, log_level):
        for line in iter(pipe.readline, b''):
            ...
            if log_level == logging.INFO:
                self.case_logger.info('[Shell] {}'.format(line))
            if log_level == logging.DEBUG:
                self.case_logger.debug('[Shell] {}'.format(line))
...
def args_parse():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
                                     description='Determine if the new crashes are from the same root cause of the old one\n'
                                                 'eg. python3 crash.py -i 5fcfdc26bc84536f79bd')
