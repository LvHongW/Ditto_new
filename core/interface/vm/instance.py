...
class VMInstance:
    ...
    def setup(self, port, image, linux, arch="amd64", mem="2G", cpu="2", key=None, gdb_port=None, mon_port=None, opts=None, timeout=None):
        ...
        if arch == "arm64":
            # arm64 guests use the virt machine and an arm64 kernel image.
            cur_opts = ["root=/dev/vda", "console=ttyAMA0"]
            self.cmd_launch = ["qemu-system-aarch64", "-machine", "virt", "-cpu", "cortex-a57", "-m", mem, "-smp", cpu]
        else:
        ...
        if arch == "arm64":
            self.cmd_launch.extend(["-display", "none", "-serial", "stdio", "-no-reboot",
                        "-drive", "if=none,file={}/stretch.img,format=raw,id=hd0".format(self.image),
                        "-device", "virtio-blk-device,drive=hd0",
                        "-snapshot", "-kernel", "{}/arch/arm64/boot/Image".format(self.linux),
                        "-append"])
        else:
